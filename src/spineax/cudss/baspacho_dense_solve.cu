...
// Suppress nvcc warnings from Eigen/XLA headers in __host__ __device__ functions
...
static ffi::Error BaspachoGpuExecute(
                           ...
                           s.totalDataSize_ * sizeof(scalar_t), str));
  {
    ...
    denseToCoalescedKernel<scalar_t><<<blocks, threads, 0, str>>>(
        J_dev, s.devData.ptr, n,
  ...
  // 3. Permute RHS on GPU: devVec[perm[i]] = f[i]
  {
    ...
    permuteForwardKernel<scalar_t><<<blocks, threads, 0, str>>>(
        f_dev, s.devVec.ptr, s.devPermutation, n);
  }
  ...
  // 5. Unpermute solution directly to output: x_out[i] = devVec[perm[i]]
  {
    int threads = 256;
    int blocks = (static_cast<int>(n) + threads - 1) / threads;
    permuteInverseKernel<scalar_t><<<blocks, threads, 0, str>>>(
        s.devVec.ptr, x_dev, s.devPermutation, n);
  }
